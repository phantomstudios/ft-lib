...
/*  Currently this implements:
    1) FT Consent cookies monitoring to enable/disable permutive tracking (with localhost/QA 'set cookie' to simulate production FT cookie preferences to help testing)
*/

export class consentMonitor {
  protected _consent: boolean = false
  ...
  init = () => {
    ...
    if (
      ...
      window.location.hostname.includes("phq")
    ) {
      const oCookieMessage = document.getElementsByClassName(
        "o-cookie-message"
      )[0];
      if (oCookieMessage) {
        const onCookieMessageAct = () => {
          console.log("SETTING LOCAL/QA FT CONSENT COOKIES");
          //set consent cookies for local/phq
